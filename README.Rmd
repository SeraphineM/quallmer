...
The package uses a hybrid pkgdown build strategy to avoid API key issues in CI:
...
The `deploy-articles` target safely updates only the `articles/` directory on gh-pages without affecting other site content. Note that `make articles`, `make site`, and `make deploy-articles` automatically knit README.md before running, so you don't forget to update it.
