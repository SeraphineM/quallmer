...
Two settings shape how much there is to backfill. The first is what a failure does to the rest of the run, and it depends on the path. On the structured path ellmer's default applies, `on_error = "return"`: the run stops sending new requests after the first failure and returns what it has, so one timeout can leave most of a corpus uncoded. On the JSON path quallmer sets `on_error = "continue"` unless you say otherwise, so every unit is attempted and only the units that actually failed are left for a backfill. Pass `on_error` yourself to choose either behaviour on either path. The second is `qlm_code(backfill = 2)`, which runs the passes in the same call, if you would rather not make them a separate step. Either way, a replication made with `qlm_replicate()` replays the passes recorded on its parent, so both are complete on the same terms.
