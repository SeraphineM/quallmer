...
**Retries happen during the run, one request at a time.** Two kinds are at work before a row is written. ellmer resends a request that a rate limit or a server error turned away, a few times with a pause between; this is transport-level, applies to every provider, and `options(ellmer_max_tries = )` sets the count. On the JSON path, `qlm_code()` also re-requests a response that arrived intact but does not fit the codebook's schema, up to `json_retries` more times. A unit still failed after these is what `qlm_failures()` shows.
...
|---|---|---|
| Rate limit, timeout, or transient server error | ellmer retries the transport request | `options(ellmer_max_tries = n)` |
| Empty, malformed, refused, or schema-invalid response on the JSON path | quallmer retries that unit during the run | `json_retries = n` |
