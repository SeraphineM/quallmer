...
The quallmer trail system creates complete audit trails following Lincoln and Guba's (1985, pp. 319-320) concept for establishing trustworthiness in qualitative research. It automatically captures the full decision history of your coding workflow, supporting the confirmability and dependability criteria that allow others to trace the logic of your analytical decisions.

## Lincoln and Guba's audit trail framework

[Lincoln and Guba (1985, p. 319-320)](https://ethnographyworkshop.wordpress.com/wp-content/uploads/2014/11/lincoln-guba-1985-establishing-trustworthiness-naturalistic-inquiry.pdf) describe six categories of audit trail materials:
...
The quallmer package operationalizes this framework for LLM-assisted text analysis:
...
First, let's load the package and create some example coded objects to demonstrate the trail system:
```{r load-package}
library(quallmer)
```

In a real workflow, you would use `qlm_code()` and `qlm_replicate()`:
...
For this tutorial, we'll create mock coded objects that show what the trail system captures:

```{r create-mock-objects}
...
- **my_analysis.rds**: Complete trail object with all coded data (reloadable with `readRDS()`)
- **my_analysis.qmd**: Quarto document with full audit trail documentation
...
When you include comparison objects, the trail captures the complete assessment workflow:
...
The trail now includes comparison information showing which runs were compared and the inter-rater reliability metrics.
...
The generated Quarto report serves as comprehensive replication materials. The replication section includes:

- **Environment setup**: Package installation and API credential configuration
...
This makes it easy for other researchers to reproduce or extend your analysis.

## Summary

The `qlm_trail()` function provides workflow traceability following Lincoln and Guba's (1985) audit trail concept:

1. **Automatic tracking**: Every `qlm_code()` and `qlm_replicate()` captures the full decision history
2. **Simple extraction**: `qlm_trail()` reconstructs your complete audit trail
3. **Easy archiving**: Add `path` to save RDS and generate Quarto report with replication instructions
