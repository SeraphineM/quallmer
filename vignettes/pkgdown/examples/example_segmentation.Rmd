...
We draw two reviews from `data_corpus_LMRDsample`, the 200-review subset of the Large Movie Review Dataset bundled with quallmer (Maas et al., 2011): one negative and one positive.
...
How reproducible is the ABSA segmentation? We re-segment the same two reviews with a second model and compare the two runs using `qlm_compare()`, which computes Krippendorff's `_u_α` for unitizing (Krippendorff, 2019, section 12.6).
