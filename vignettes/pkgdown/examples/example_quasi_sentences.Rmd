...
`qlm_segment()` applies the Manifesto Project quasi-sentence rules to a raw manifesto, producing a quanteda corpus in which each document is a single political statement. The `reason` docvar makes it possible to audit both split and non-split decisions against the handbook rules. The segmented corpus can feed directly into `qlm_code()` for domain-level coding of each quasi-sentence, reproducing the full Manifesto Project pipeline within a single R workflow.
