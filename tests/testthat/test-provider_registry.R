...
test_that("endpoint-only backfill changes retry terminal failures and replay the route", {
  local_registry()
  qlm_register_provider("gateway", "https://original.example/v1", "ORIGINAL_KEY")
  seen <- character()
  local_mocked_bindings(structured_chat_turns = function(chat, prompts, type, batch, execution_args) {
    seen <<- c(seen, chat$get_provider()@base_url)
    list(json_turn(list(label = "yes")))
  })
  x <- qlm_code("text", registry_codebook(), "gateway/kimi", structured = "structured")
  x$label <- NA_character_
  x$.error <- list(simpleError("The response was cut off at the max_tokens limit"))
  unchanged <- suppressMessages(qlm_backfill(x, passes = 1))
  expect_true(is.na(unchanged$label))
  expect_length(seen, 1)
  y <- suppressMessages(qlm_backfill(x, model = "moonshot/kimi", passes = 1))
  pass <- attr(y, "meta")$object$backfill[[1]]
  expect_identical(y$label, "yes")
  expect_identical(pass$model, "openai_compatible/kimi")
  expect_identical(pass$overrides$base_url, provider_definition("moonshot")$base_url)
  expect_identical(pass$provider_resolution$requested_model, "moonshot/kimi")
  rm("gateway", envir = provider_registry)
  replay <- suppressMessages(replay_backfill(x, y))
  expect_identical(replay$label, "yes")
  expect_identical(tail(seen, 1), provider_definition("moonshot")$base_url)
})

test_that("replication labels an endpoint override and preserves it on replay", {
  local_registry()
  qlm_register_provider("gateway", "https://original.example/v1", "ORIGINAL_KEY")
  local_mocked_bindings(structured_chat_turns = turns_stub(list(json_turn(list(label = "yes")))))
  x <- qlm_code("text", registry_codebook(), "gateway/kimi", structured = "structured")
  key <- function() Sys.getenv("REPLACEMENT_KEY")
  y <- suppressMessages(qlm_replicate(x, base_url = "https://replacement.example/v1", credentials = key))
  resolution <- attr(y, "meta")$object$provider_resolution
  expect_identical(resolution$requested_model, "gateway/kimi")
  expect_true(resolution$endpoint_overridden)
  expect_null(resolution$api_key_env)
  expect_output(print(y), "gateway/kimi \\(endpoint overridden\\)")
  trail <- qlm_trail(y)
  expect_output(print(trail), "gateway/kimi \\(endpoint overridden\\)")
  file <- tempfile(fileext = ".qmd")
  generate_trail_report(trail, file)
  report <- paste(readLines(file), collapse = "\n")
  expect_match(report, "gateway/kimi (endpoint overridden)", fixed = TRUE)
  expect_match(report, "https://replacement.example/v1", fixed = TRUE)
  rm("gateway", envir = provider_registry)
  replay <- qlm_replicate(trail$runs[[1]]$coded)
  expect_identical(attr(replay, "meta")$object$provider_resolution, resolution)
  expect_identical(attr(replay, "meta")$object$chat_args$base_url, "https://replacement.example/v1")
  expect_identical(body(attr(replay, "meta")$object$chat_args$credentials), body(key))
})
